...
async function getUserIdByProfessional(type, professionalId) {
  ...
  try {
    ...
    // The full_name join below can still match multiple dmd.professional
    // rows sharing a name, so it needs the same deterministic tiebreak used
    // in the profile pages (specialty match, then dha_unique_id) rather than
    // a bare LIMIT 1, which would non-deterministically misattribute views.
